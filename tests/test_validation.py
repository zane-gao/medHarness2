...
def test_validate_sample_run_rejects_non_object_manifest_rows(tmp_path: Path):
    """A JSONL scalar must not disappear from the manifest denominator."""
    (tmp_path / "manifest.jsonl").write_text('"not-a-case"\n', encoding="utf-8")
    _write_json(tmp_path / "summary.json", {"case_count": 1})
...
def test_validate_sample_run_reports_manifest_line_numbers_for_multiple_scalars(tmp_path: Path):
    ...
    assert result["errors"].count("invalid_manifest_jsonl:row_1:not_object") == 1
    assert result["errors"].count("invalid_manifest_jsonl:row_2:not_object") == 1
