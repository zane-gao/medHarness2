...
<main>
  <div class="metrics"><div class="metric"><b id="runCount">0</b><span>Runs</span></div><div class="metric"><b id="activeCount">0</b><span>Active</span></div><div class="metric"><b id="toolCount">0</b><span>Tools</span></div><div class="metric"><b id="modelCount">0</b><span>Model statuses</span></div><div class="metric"><b id="validatedCount">0</b><span>Validated studies</span></div></div>
  <div class="layout"><div>
    ...
    <section><h2>Model Status</h2><div class="table-wrap"><table><thead><tr><th>Model</th><th>Resource</th><th>Runtime</th><th>Validation</th><th>Inputs</th><th>Evidence</th><th>Blocked reason</th></tr></thead><tbody id="modelStatuses"></tbody></table></div></section>
...
async function loadModels(){const data=await fetch('/catalog/models?config_path=config/dmx_strong.yaml').then(r=>r.json());const rows=data.model_statuses||[];modelCount.textContent=rows.length;modelStatuses.innerHTML=rows.map(x=>{const evidence=x.latest_evidence||{};return `<tr><td><code>${esc(x.model_key)}</code><br>${esc(x.adapter)}</td><td class="status-${esc(x.status)}">${esc(x.status)}</td><td>${esc(x.runtime_state)}</td><td>${esc(x.validation_state)}</td><td>${esc((x.input_capabilities||[]).join(', '))}</td><td>${esc(evidence.type||'none')}<br><code>${esc(evidence.path||'')}</code></td><td>${esc(x.blocked_reason||'')}</td></tr>`}).join('');}
...
async function refresh(){await Promise.all([loadRuns(),loadTools(),loadModels(),loadExperiments()]);}
