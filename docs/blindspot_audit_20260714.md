...
> **2026-07-17 research manifest contract 增量**：`research prepare-manifests` 不再只信任 pilot `manifest.jsonl` 的字符串字段；现在会 fail-closed 校验 `annotation_path` 必须为 `cases/` 下的相对路径、目标文件存在且符合 `AnnotationCase` contract，并核对 `pilot_case_id` 与模态一致。新增缺失文件和身份漂移回归测试；真实 pilot10 仍为 `blocked`（缺临床 gold/provider runs），没有把阻塞状态升级成 winner。
...
- 前端视觉回归已实际尝试：隔离安装 `@playwright/test` 并下载 Chromium 后，桌面/移动两项均因宿主缺少 `libnspr4.so` 无法启动；因此保持“未验证”，不把浏览器启动失败误记为页面功能失败或测试通过。
