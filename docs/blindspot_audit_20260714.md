...
- **OCR 地基质量门禁**：页级 OCR 现在输出 `quality_status=passed|review_required|blocked`；空结果和疑似截断结果会被标记为 `blocked`，`prepare_sample_dataset` 不再把其缓存路径写入可用 `report_text`。独立 verifier 的 disagreement、失败或非法响应只标记 `review_required`，主 OCR 文本保持不变。`ocr-benchmark` 对带 provenance 的低质量 candidate sidecar 同样 fail-closed，不会把 blocked/review 文本纳入 CER 或 provisional winner。
...
- 回归测试持续新增；截至本轮验证，最新全量回归为 `598 passed, 19 warnings`，并已通过专项 OCR 回归、`compileall` 与 `git diff --check`。M9 失败分母已接入 `analyze_run` JSON/Markdown 汇总，workflow2/workflow3 聚合边界及内部一致性也已纳入合约验证；pilot10 标注状态已有可执行验收门禁；面板构建器还会忽略自身 `web/index.html` 的生成差异，避免正常重建误报 dirty。
