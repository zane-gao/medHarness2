...
> **2026-07-16 后续增量**：`dmx_strong.yaml` 已提供 Doubao Seed 候选主 OCR（`ocr_primary`）和独立 Qwen OCR verifier（`ocr_verifier`）路由；两者只作为冻结 benchmark 候选，不能据模型名称直接宣布 winner。多页 OCR verifier 现在按保留页逐页抽查并保留页级审计结果。全量回归测试为 394 passed、18 warnings。
