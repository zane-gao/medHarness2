...
> **最新复核（2026-07-17）**：当前主分支回归为 `1851 passed, 20 warnings`。正式 `research run-ocr` 仍因真实候选/凭据缺失保持 blocked；另有 `/tmp` 中未注册的 Qwen exploratory sidecar，已核验 pilot10 首轮 8/10，针对两例的定向重试为 2/2，但不计入正式双重复 benchmark、winner 或论文统计。下文更早的 `1718/1785/1801/1836/1847/1850` 等数字均为历史快照。
...
> **2026-07-17 审计页列表边界收口**：verifier audit 的 `pages` 为空或混入非对象时统一判为 `blocked`，不会因过滤非法项后恰好剩余 `agree` 而误通过；全量回归曾为 `1785/1801/1835/1836/1847/1850 passed, 20 warnings`，当前基线为 `1851 passed, 20 warnings`。
