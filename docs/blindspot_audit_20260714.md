...
- 回归测试持续新增；2026-07-16 本轮全量验证为 `443 passed, 18 warnings`，并已通过 `compileall` 与 `git diff --check`。M9 失败分母已接入 `analyze_run` JSON/Markdown 汇总，workflow2/workflow3 聚合边界及内部一致性也已纳入合约验证；pilot10 标注状态已有可执行验收门禁。
