...
> **最新复核（2026-07-17）**：当前主分支回归为 `1828 passed, 20 warnings`。真实 `research run-ocr` 已核验 pilot10 的 10/10 源 PDF 映射，但在缺少 Doubao/Qwen 凭据和 PaddleOCR-VL 运行时的环境中保持 blocked；这不是 OCR winner 或临床标注完成证据。下文更早的 `1718/1785/1801` 等数字均为历史快照。
...
> **2026-07-17 审计页列表边界收口**：verifier audit 的 `pages` 为空或混入非对象时统一判为 `blocked`，不会因过滤非法项后恰好剩余 `agree` 而误通过；全量回归曾为 `1785/1801 passed, 20 warnings`，当前基线为 `1828 passed, 20 warnings`。
