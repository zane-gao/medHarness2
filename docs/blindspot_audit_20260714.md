...
> **最新复核（2026-07-17）**：当前主分支回归为 `1865 passed, 22 warnings`。正式 `research run-ocr` 仍因真实候选/凭据缺失保持 blocked；未注册的 Qwen exploratory 第三轮 10 例为 `3/10 passed`、`7/10 review_required`、`0/10 blocked`，不计入正式双重复 benchmark、winner 或论文统计。新增 2 个 warning 来自 Paddle 依赖导入提示。下文更早的 `1718/1785/1801/1836/1847/1850/1851/1857/1859/1864` 等数字均为历史快照。
...
> **2026-07-17 Paddle 权重门禁修复**：A40 已安装 PaddleOCR/Paddle runtime，但 `PaddleOCR-VL-1.6` 权重尚未完整落地；研究运行器现在在执行前检查权重缓存，返回 `paddle_model_weights_unavailable`，不再把模型初始化时的网络/文件异常归类为泛化的 provider error。
...
> **2026-07-17 运行门禁复核**：真实数据路径上的 `workflow preflight --require-real-ocr` 已确认 52 例与 `cxr/ct/mri=20/25/7` 路由覆盖；真实 OCR/verifier 当前因可确认的 provider/model 不可用、DMX 实测 401/凭据问题及 PaddleOCR-VL runtime 未就绪而阻断。`annotation validate` 对 pilot10 返回 `not_started`、`0/10` 并以非零退出；这两类阻断均未被渲染或统计为成功。

> **2026-07-17 研究准备 CLI 语义收口**：`research prepare-manifests` 现在把“生成 blocked/pending 的研究清单”视为准备成功并返回 0；只有 `research run-ocr` 等实际证据执行命令在 provider/模型证据缺失时返回非零。新增 CLI 回归覆盖，避免自动化把准备阶段误判为实验失败。
...
> **2026-07-17 教育链路收尾**：教育建议工作流现在要求 finding、hazard、alignment 和 pairwise comparison 输入为对象列表/对象；损坏字符串不会再被 `list()` 拆成字符后进入教学建议。专项教育/API 回归通过，全量回归更新为 `1118 passed, 20 warnings`。

> **2026-07-17 分析链路收尾**：`analyze-run` 现在对 workflow1 的 generated report 字段执行严格边界校验；模型、来源、报告、模态、证据层必须是字符串，warnings 必须是字符串列表，metadata 必须是对象，损坏字段会 fail-closed 而不会进入模型/模态统计。分析专项回归 `56 passed`，全量回归更新为 `1122 passed, 20 warnings`。

> **2026-07-17 重评估链路收尾**：`reevaluate-run` 现在严格校验 `workflow1.input`、`workflow1.input.prepared_assets` 与 `human_evaluation.composite_inputs` 必须是对象；键值对列表等损坏结构会记录为失败病例，不再由 `dict()` 隐式转换后继续评估。重评估/分析/主设计回归 `135 passed`，全量回归更新为 `1124 passed, 20 warnings`。

> **2026-07-17 批处理入口收尾**：`run_batch_readers(limit)` 现在要求非负整数，负数、小数、字符串和布尔值不会再通过 Python 切片改变病例分母。专项回归 `100 passed`，全量回归更新为 `1128 passed, 20 warnings`。

> **2026-07-17 合并汇总收尾**：`merge-batches` 统计 generated reports 时现在严格要求报告为对象，模型/来源/报告/模态/证据层为字符串，warnings 为字符串列表，metadata 为对象；损坏产物会 fail-closed，不再污染合并后的 provenance 计数。合并/批处理/重评估回归 `109 passed`，全量回归更新为 `1132 passed, 20 warnings`。

> **2026-07-17 部门汇总收尾**：`department` 工作流现在严格校验 `per_reader`、`denominator`、`cases` 以及每个 reader 的 payload 和 `modelwise_metrics` 必须是对象结构；非法聚合输入不会再通过 `dict()` 或字符迭代进入读者分位数、分母和模型统计。专项聚合回归 `125 passed`，全量回归更新为 `1136 passed, 20 warnings`。

> **2026-07-17 批处理评估对象收尾**：`batch-readers` 现在严格校验 `human_evaluation`、`composite_inputs` 和 `generated_evaluations` 为对象/对象列表；损坏评估不会再通过 `dict()` 进入 reader/modelwise 统计。批处理/合并/重评估回归 `117 passed`，全量回归更新为 `1140 passed, 20 warnings`。

> **2026-07-17 合并病例身份收尾**：`merge-batches` 现在严格校验每个 batch 的 `cases`/`failed_cases` 为对象列表，`case_id` 与 `reader` 为字符串；非法身份不会被 `str()` 隐式转换而造成病例去重或读者聚合漂移。合并专项回归 `121 passed`，全量回归更新为 `1144 passed, 20 warnings`。
...
> **2026-07-17 单病例资产收尾**：`single_case` 现在严格校验 `prepared_assets` 为对象，`primary_image` 与 `volume_path` 若存在必须是字符串；非法列表、数字或字符串资产不会再通过 `str()` 变成伪路径。单病例/批处理/重评估/CLI 回归 `172 passed`，全量回归更新为 `1154 passed, 20 warnings`。

> **2026-07-17 Tool2 抽取候选收尾**：Tool2 的 CXR 归一化与 fallback 路径现在严格校验 `findings` 为对象列表、`metadata` 为对象、`warnings` 为字符串列表，损坏候选不会再被 `list/dict` 半合法化后进入 FindingGraph 或 OCR 统计。Tool2/抽取/本体专项回归 `169 passed`，全量回归更新为 `1157 passed, 20 warnings`。
...
> **2026-07-17 OCR 缓存收尾**：OCR 缓存复用前新增 sidecar 结构门禁；`warnings` 必须是字符串列表，`verifier`/`quality_audit` 必须是对象，`quality_status` 只能是 `passed/review_required/blocked`。损坏缓存现在强制重新生成，不会把错误审计状态带入可用 OCR 文本；PDF 文本层缓存不再因无关 VLM 路由变化而失效。当前全量回归为 `1718 passed, 20 warnings`。
...
> **2026-07-16 benchmark 复现快照增量**：正式 benchmark 的 route/config snapshot 现在显式记录每个 LLM role 的 `consistency_runs`、schema 尝试次数和传输重试次数；这些运行参数变化会使旧 artifact 校验失败并要求重跑，避免配置漂移下静默复用旧结果。
...
> **2026-07-17 交接路径修正**：pilot10 已作为 tracked 包提交到 `annotation/pilot10/`，远程检出后可直接交给两位真实 reader；`outputs/annotation/pilot10_20260717/` 不再作为远程交接路径。模型身份映射仍仅保留在本地 ignored 产物中。北川参考报告已作为当前文本 gold；真实 reader 校准尚未开始，OCR winner 和论文 formal claim 仍需 provider、质量与重复一致性证据。

> **2026-07-17 research manifest contract 增量**：`research prepare-manifests` 不再只信任 pilot `manifest.jsonl` 的字符串字段；现在会 fail-closed 校验 `annotation_path` 必须为 `cases/` 下的相对路径、目标文件存在且符合 `AnnotationCase` contract，并核对 `pilot_case_id` 与模态一致。新增缺失文件和身份漂移回归测试；北川参考报告已登记为当前文本 gold，pilot10 OCR 仍因真实 provider 双次运行未完成而 blocked，没有把阻塞状态升级成 winner。

> **2026-07-17 OCR sidecar 文本绑定增量**：新写入的 OCR sidecar 记录缓存文本 `text_sha256`，复用前会校验 sidecar 与文本内容一致；文本被替换、sidecar 类型损坏或质量摘要与 verifier 页状态矛盾时会 fail-closed 重新执行 OCR，不再静默复用错配缓存。OCR 专项回归为 `186 passed`。
...
**H6. `select_top_k` 不能把不确定性内的点估计硬切成确定赢家** `tools/tool9_rank.py:21`
历史实现只按点估计排序，N=11 下 0.72 vs 0.71（差在抽样噪声内）会被严格排序，0.72 进 pairwise、0.71 丢。当前实现已兼容读取候选的 `score_ci_lower/score_ci_upper`（或各排名指标 CI 的保守加权区间）：若与 cutoff 区间重叠，候选会保留并标记 `uncertainty_overlap=true`、`requires_review=true`；没有 CI 时不伪造区间，继续使用既有 `near_cutoff_tolerance` 机制。真实冻结集仍需报告实际不确定性并决定正式 promotion。
...
### 2026-07-16 增量：catalog 配置错误结构化

- `/catalog/model-roles` 与 `/catalog/tools` 现在捕获配置解析异常并返回 JSON 形式的 HTTP 500，而不是 Starlette 默认纯文本错误。
- 新增控制面 API 回归，覆盖 malformed YAML；正常 catalog 输出保持兼容。

### 2026-07-16 增量：sample/preflight 配置错误闭环

- `/workflow/sample-data` 与 `/workflow/preflight` 的配置加载已纳入异常闭环；malformed YAML 现在返回结构化 HTTP 500，并写入失败 registry。
- 新增两条 API 回归；正常空样本和门禁阻断语义保持不变。

### 2026-07-16 增量：experiments 查询异常闭环

- `GET /experiments` 现在捕获实验就绪汇总异常并返回结构化 HTTP 500；正常 pilot/validated 状态协议保持兼容。
- 新增控制面回归覆盖底层异常路径。

### 2026-07-16 增量：control-plane 入口异常闭环

- `/control-panel`、`POST /runs`、`GET /runs` 现在捕获面板渲染或控制数据库异常，并返回结构化 HTTP 500。
- 运行控制已有的 404/409 状态语义保持不变；新增回归覆盖存储和渲染异常。

### 2026-07-16 增量：run 控制存储异常闭环

- `GET /runs/{run_id}`、`POST /runs/{run_id}/cancel`、`POST /runs/{run_id}/retry` 现在捕获底层控制数据库异常并返回结构化 HTTP 500。
- 不存在 run 的 404 与非法状态转换的 409 保持不变；新增存储异常回归。

### 2026-07-16 增量：路由部位软门禁收尾

- 生成质量门禁现在将 `body_part_mismatch` 保留为可审计 warning，但不再据此淘汰候选；模态冲突和 fallback 仍保持阻断。
- 这与三模态模态优先路由一致，避免部位词误报重新造成硬路由；新增回归验证部位冲突仍可进入排序、模态冲突仍失败。
- 新增 single-case 端到端回归，确认仅部位冲突的候选仍进入 ranking；本轮全量回归目标更新为 `571 passed, 18 warnings`。

### 2026-07-16 增量：CLI 配置异常闭环

- CLI `workflow single-case` 与 `dashboard build` 现在将配置加载纳入异常处理；malformed YAML 会写入失败 registry、记录 `exception_type` 并返回非零退出码。
- 正常执行参数和成功输出保持兼容，避免 CLI 与 API 在同类配置错误上出现不同状态语义。

### 2026-07-16 增量：CLI preflight/education 异常闭环

- CLI `workflow preflight` 与 `workflow education` 现在也将配置加载和底层执行纳入失败 registry；malformed YAML 返回非零退出并记录 `exception_type`。
- 当前全量回归为 `585 passed, 18 warnings`。

### 2026-07-16 增量：benchmark CLI 配置异常闭环

- `benchmark plan/run/evaluate` 遇到配置解析或初始化异常时，现在分别写出结构化失败产物（plan、generation summary、evaluation summary）并返回非零退出码。
- 正常 benchmark 产物格式和成功路径保持兼容；新增 malformed YAML 回归覆盖三个入口；本轮全量回归为 `578 passed, 18 warnings`，后续入口修复后的最新全量回归为 `585 passed, 18 warnings`。

### 2026-07-16 增量：核心 workflow CLI 异常闭环

- `workflow sample-data`、`sample-full`、`batch-readers`、`reevaluate-run` 的配置加载和底层执行异常现在统一写入失败 registry，并返回非零退出码。
- `sample-full` dry-run 与正式运行的成功路径保持原有产物结构；新增四条 malformed YAML 回归，本轮全量回归为 `582 passed, 18 warnings`。

### 2026-07-16 增量：轻量 CLI 入口异常闭环

- `live-smoke`、`models list`、`tools catalog` 遇到配置解析异常时不再直接抛出默认异常：分别写失败 smoke 产物、返回非零退出，或写失败 catalog/registry。
- 新增三条 malformed YAML 回归；本轮全量回归为 `585 passed, 18 warnings`。

### 2026-07-16 增量：本地工具目录发布一致性

- 将用户已有的 `.serena/` 本地工具元数据目录加入 `.gitignore`；它不会进入仓库，也不再让工作树状态与 `baseline.dirty_worktree: false` 产生表面矛盾。
- 项目元数据测试 `3 passed`，全量回归保持 `585 passed, 18 warnings`。

### 本轮已落地

- **教育 Likert 输入门禁**：教育建议工作流现在只接受 1–5 的严格整数分数，不再把 `3.5` 等小数静默截断后参与 weakest/overall 统计。

- **Pairwise checkpoint 索引门禁**：T4 adjudication checkpoint 现在只接受非负、非布尔整数 `error_index`，避免 bool-as-int 造成 disagreement/decision 错配。

- **Tool11 hazard level 解析门禁**：按危害加权现在只接受严格整数等级（允许字符串整数），不再把 3.5 等分数静默截断成 3。

- **正式 hazard level 汇总过滤**：`adjudicated_hazard_levels` 与 `consensus_hazard_levels` 只接受 1–5 的非布尔整数，非法列表项不会进入 Counter 或分布展示。

- **Tool4 索引类型门禁**：最小 hazard judge candidate 不再透传布尔 `alignment_error_index`，避免 bool-as-int 污染证据索引和重试提示。

- **Hazard 类型门禁**：benchmark evaluation 现在将 `bool` 排除在 `hazard_level` 与 `error_index` 之外，避免 Python 的 bool-as-int 语义污染 T4 覆盖、consensus 和正式 hazard 汇总。

- **Tool1 均值输入门禁**：`likert_mean()` 现在只接受有限、非布尔且位于 1–5 的分数，历史产物中的 bool、NaN、Inf 和越界值不会污染均值。

- **正式 Welch 输入门禁**：benchmark 统计比较现在拒绝 bool、NaN 和 Inf 测量值，与 Tool12 的 fail-closed 规则一致。

- **正式 hazard 汇总回归保护**：新增测试锁定多个 adjudication abstain 按决策条数计数，避免重复聚合导致平方计数；当前实现已通过该契约。

- **H6 CI 量纲边界**：Tool9 对指标级 CI 现在按指标量纲归一化（`likert_mean` 的 1–5 CI 使用 `(x-1)/4`），总分 CI 仅接受 0–1；非法量纲不再被静默裁剪。
- **OCR 地基质量门禁**：页级 OCR 现在输出 `quality_status=passed|review_required|blocked`；空结果和疑似截断结果会被标记为 `blocked`，`prepare_sample_dataset` 不再把其缓存路径写入可用 `report_text`。独立 verifier 的 disagreement、失败或非法响应只标记 `review_required`，主 OCR 文本保持不变，且 review_required 文本不会写入可用 `report_text`。`ocr-benchmark` 对带 provenance 的低质量 gold/candidate sidecar 同样 fail-closed，不会把 blocked/review 文本纳入 CER 或 provisional winner。
...
- **Tool4 / 生成器注册表参数门禁**：`evaluate_hazards` / `review_hazards` 的 `max_retries`、`consistency_runs` 以及生成器注册表的 `max_new_tokens`、`timeout_sec` 现在拒绝 bool、小数、字符串和非正数，不再通过直接调用或 YAML 注册表隐式转换污染重试/生成配置。
- **正式 benchmark 聚合计数门禁**：T5 `retained_error_count`、T5 rejected/modified/abstained 计数、T4 agreement 计数和验证后的 attempt count 现在统一 fail-closed；非法聚合产物不会进入正式 summary。
- **Tool4 重试参数旁路门禁**：primary、reviewer 和第三仲裁器的 `max_retries` 现在都在入口校验；即使空候选快速返回、无 disagreement 或走 deterministic fallback，也不会绕过非法 bool/小数/字符串/非正数输入。
- **生成器构造层门禁**：`GeneratorEntry` 自身现在校验 `max_new_tokens` 与 `timeout_sec`；直接实例化、legacy 适配器和 YAML 注册表都不能绕过正整数约束。
- **Dashboard 汇总门禁**：`summarize_dashboard_payload` 对病例、实验和图表数量统一拒绝 bool、小数、字符串和负数，前端 KPI 不再把非法产物静默转换成正常计数。
- **Dashboard 质量门禁显示门禁**：`_format_gate_status` 对 `passed/total` 计数也统一 fail-closed，质量门禁状态卡不会把非法计数渲染成可信的通过比例。
- **Dashboard CSV 计数显示门禁**：模型报告数和质量失败数现在接受合法整数文本、拒绝小数/布尔/负数/非法字符串，展示层不会把坏产物静默显示为 0。
- **OCR 缓存 verifier provenance**：OCR 缓存现在同时绑定 verifier 是否配置、provider 和 model；从“有 verifier”切换到“无 verifier”或反向切换都会重新生成，避免过期审计状态污染当前质量门禁。
- **OCR verifier 状态枚举门禁**：verifier 只接受 `agree` 或 `disagreement`；未知状态、缺失状态和非 JSON 响应都标记为 `invalid_verifier_response`，不会把不确定审计误报为 `passed`。
- **Tool4 hazard 归一化门禁**：危害等级归一化现在只对严格整数执行边界裁剪；布尔、小数、字符串和非法类型回退到错误类型默认等级，避免 legacy/deterministic 产物通过 `int()` 污染统计。
- **workflow 产物整数门禁**：sample-full 汇总、experiments registry 和 pairwise T5 checkpoint 不再对 JSON 产物直接调用 `int()`；布尔、小数、字符串和负数会 fail-closed，避免外部产物污染病例、实验和确定性错误计数。本轮新增回归后全量测试为 `798 passed, 20 warnings`。
- **Tool1/2/5/6 参数与 PDF 页数门禁**：Likert、finding extraction、alignment audit、structure audit 的重试/分块参数，以及本地 VLM PDF 页数限制，现在均拒绝 bool、小数、字符串和非正数；sample-full 的 workflow3/reader 汇总也改为严格非负整数。全量回归提升至 `823 passed, 20 warnings`。
- **控制面板与聚合合约严格计数门禁**：RunStore 的 `list_runs(limit)` 不再隐式转换分页参数；workflow2/workflow3、denominator、reader 和 percentile 的 Pydantic 计数字段改为 `StrictInt`，字符串/布尔计数不会再被自动转换后进入验证或统计。本轮全量回归为 `833 passed, 20 warnings`。
- **评测合约严格整数门禁**：TextSpan 索引、HazardJudgement/Review/Adjudication 的 error index、hazard level、level delta、计数，以及 Alignment 审计索引和 Structure Audit clinical impact 全部改为 `StrictInt`；评测产物中的字符串/布尔/小数不再被 Pydantic 自动转换。本轮全量回归为 `836 passed, 20 warnings`。
- **评测与聚合浮点门禁**：Measurement value/normalized_mm、FindingGraph coverage、Hazard/Alignment/Structure confidence、aggregate rate/score/percentile 等浮点字段改为 `StrictFloat`，字符串和布尔值不会再被自动转换进入统计或质量门禁；正常整数/浮点数仍保持兼容。本轮全量回归为 `838 passed, 20 warnings`。
- **API 请求体整数门禁**：SingleCase、sample/preflight、sample-full、batch readers、merge/validate 等请求体的 `top_n`、`limit`、`expected_cases` 改为 `StrictInt`；JSON 字符串和布尔值不会再在 API 入口被自动转换后绕过下游校验。本轮全量回归为 `847 passed, 20 warnings`。
- **CaseManifest 列表边界门禁**：manifest 的 `image_paths` 与 `warnings` 现在必须是字符串列表；字符串不再被 `list()` 拆成字符、字典不再被当作 key 列表，非法外部 manifest 会 fail-closed。本轮全量回归为 `854 passed, 20 warnings`。
- **迁移/reevaluate 数值边界门禁**：旧产物迁移的 `_bounded_int` 不再把字符串、小数或布尔值静默转成危害等级；reevaluate 的 `expected_cases` 只接受真实非负整数，异常值按缺失处理，不再改变实验分母。本轮全量回归为 `856 passed, 20 warnings`。
- **迁移 warning 列表边界门禁**：旧 generated report 与 case 的 warning 字段现在支持单个字符串作为一个 warning，但拒绝字典、混合类型和非字符串列表；不会再用 `list()` 把字符串拆成字符或把字典 key 当作 warning。本轮全量回归为 `858 passed, 20 warnings`。
- **迁移 case 顶层对象列表门禁**：`generated_reports`、`generated_evaluations`、`pairwise_comparisons` 和 `rankings` 现在必须是对象列表；单个字符串、字典或包含非对象元素的列表会 fail-closed，不再被迁移器拆分或半合法化。本轮全量回归为 `870 passed, 20 warnings`。
- **analyze_run workflow1 对象列表门禁**：分析汇总读取 `generated_reports`、`rankings` 和 `pairwise_comparisons` 时现在要求对象列表，异常字符串/字典/混合列表会 fail-closed，不再被 `list()` 拆分后污染 dashboard 与统计。本轮全量回归为 `879 passed, 20 warnings`。
- **benchmark 评测对象列表门禁**：真实 LLM case verification 与 benchmark metrics 读取 `generated_evaluations`、`pairwise_comparisons`、T5 adjudicated candidates、T4 disagreements/decisions/errors 时现在要求对象列表，异常字符串、字典和混合列表会 fail-closed。本轮全量回归为 `885 passed, 20 warnings`。
- **路由注册表列表字段门禁**：模型配置的 `supported_modalities`、`supported_body_parts`、`python_paths` 及 legacy 输出 warnings 现在必须是字符串列表；错误的标量、字典或混合列表不会被拆成字符后影响模态命中和候选排序。本轮全量回归为 `894 passed, 20 warnings`。
- **Tool5/alignment 对象列表门禁**：候选/参考 findings、alignment 的 matched/approximate/mismatched/error_candidates，以及 pairwise 的 adjudicated candidates 现在必须是对象列表；malformed 字符串、字典或混合列表会在入口明确失败，不再进入 matcher、audit 或 hazard 统计。本轮全量回归为 `900 passed, 20 warnings`。
- **路由布尔配置门禁**：生成器注册表的 `ready`、`report_trained`、`fresh_inference` 现在只接受真实布尔值；字符串 `"false"` 不会再被 Python 的 `bool()` 错误解释为 `True`，避免未就绪模型进入正式路由。本轮全量回归为 `915 passed, 20 warnings`。
- **OCR benchmark 文本类型门禁**：OCR 冻结集评测只接受字符串、合法文本路径或包含字符串 `text/ocr_text` 的对象；布尔、数字、列表和错误对象不再被 `str()` 转成伪 OCR 文本参与评分，而是进入 blocked。本轮全量回归为 `920 passed, 20 warnings`。
- **OCR manifest 身份字段门禁**：OCR 冻结集的 `case_id` 只接受非空字符串；候选模型 key 不再把非字符串值静默转成身份字符串，避免覆盖率、provenance 和模型选择错配。本轮全量回归为 `925 passed, 20 warnings`。
- **Tool9 排名控制参数门禁**：`top_k`、cutoff 容差和 metric weights 现在只接受严格的正整数/有限非负数/非空字符串键映射；布尔、字符串、负数、非有限值和全零权重会在排名入口 fail-closed，不再通过 `int/float/bool` 隐式转换改变候选排序或复核范围。专项回归为 `200 passed`。
- **统计 provenance 布尔门禁**：Tool9/10/11/12、质量门禁、batch-readers、merge、reevaluate 和 benchmark verification 现在只接受真实布尔 `fallback_used`；数字、字符串和其他异常值统一按不可信/回退处理，不会进入正式排名、统计或质量通过路径。全量回归达到 `933 passed, 20 warnings`。
- **CaseManifest 身份字段门禁**：`case_id/id`、`reader/radiologist_id`、`modality` 和 `body_part` 在显式提供时必须是字符串；缺失字段仍保留历史默认值。字典、数字、布尔和列表不会再被 `str()` 转成合法身份，避免 OCR 绑定、路由和 reader 统计被污染。
- **CaseManifest 路径/对象字段门禁**：`report_pdf`、`report_text/report_text_path` 和 `volume_path` 在显式提供时必须是字符串；`derived_assets` 与 `metadata` 必须是对象。异常值不会再被 `str()`/`dict()` 半合法化或抛出未归类的 `TypeError`，而是统一以 manifest 校验错误阻断。
- **artifact 复用身份与文本门禁**：遗留生成 JSONL 中的 `case_id/sample_id`、报告文本和模态字段现在必须保持字符串；非对象行、错误身份、错误报告类型或错误模态会显式返回 artifact 校验失败，不会被 `str()` 转成可用报告或误配到其他病例。
- **GeneratedReport 迁移字段门禁**：旧版生成报告迁移对显式 `model/source/report/modality` 非字符串值和非对象 `metadata` 现在明确失败；缺失字段仍使用历史默认值，不再通过 `str()`/`dict()` 半合法化旧产物。
- **FindingGraph 迁移字段门禁**：旧 finding graph 迁移对显式非字符串 `modality/backend` 和非对象 `metadata/template_coverage` 现在明确失败；缺失字段继续使用历史默认值，避免异常临床图谱字段被隐式转换。
- **Finding 字段迁移门禁**：旧 finding graph 中的 `finding_id/id`、观察文本和 `source_text` 现在必须是字符串；异常字典、数字、布尔和列表会明确阻断，不再被 `str()` 转成可对齐的临床观察或 finding 身份。
- **Finding/Hazard 列表门禁**：finding graph 的 `missing/warnings` 与 hazard judgement 的 `evidence_ids` 现在必须是字符串或字符串列表；混合类型、字典、数字和布尔不会再被 `str()` 转换进入缺失项、警告或证据绑定。
- **Hazard 迁移文本/布尔门禁**：旧 hazard result 的 `error_type/explanation/recommended_action` 显式值必须是字符串，`abstain` 必须是真实布尔；异常值不会再被 `str()`/`bool()` 转成临床危害判断或改变是否需要仲裁。
- **Finding/Hazard 对象字段门禁**：finding 的 `attributes` 与 hazard result 的 `metadata` 显式值必须是对象；字符串、列表、数字和布尔会明确失败，不再触发未分类 `TypeError` 或被 `dict()` 半合法化。
- **分析/重评估病例身份门禁**：`analyze_run` 对 workflow2 中显式非字符串 `case_id/reader/modality/body_part` 直接 fail-closed；`reevaluate_run` 则将该病例记录为失败并跳过重评估，避免异常身份被 `str()` 转入统计和 reader 聚合。
- 回归测试持续新增；截至本轮验证，最新全量回归为 `1112 passed, 20 warnings`，并已通过专项 OCR 回归、`compileall` 与 `git diff --check`。M9 失败分母已接入 `analyze_run` JSON/Markdown 汇总，workflow2/workflow3 聚合边界及内部一致性也已纳入合约验证；pilot10 标注状态已有可执行验收门禁；模型角色、顶层 LLM 和 per-call provider 参数的重试/超时/seed/max_tokens 配置已拒绝隐式整数转换，显式零值不会被默认值吞掉；正式 benchmark 汇总中的 checkpoint、fallback、hazard、T5、consensus、attempt 和 agreement 计数也已改为缺失按 0、显式非法即 fail-closed；Tool4 三条评审路径的重试参数也已覆盖快速返回、无 disagreement 和 fallback 旁路；Tool4 hazard 归一化对严格整数执行边界裁剪、对非法类型回退默认等级；生成器注册表和 `GeneratorEntry` 构造层的生成限制参数、Dashboard summary、质量门禁状态和 CSV 展示的病例/实验/图表、passed/total、报告数及质量失败数也统一拒绝隐式转换；OCR 缓存 verifier provenance 已绑定，切换 verifier 配置不会复用旧质量状态；OCR verifier 状态限定为 `agree/disagreement`，未知状态不会升级为 passed；preflight、department 分母、experiments 面板计数字段和 dashboard KPI/health-strip 和 API registry 指标和 CLI validate-run/preflight、其余 workflow registry、education reader case_count、API analyze-run、OCR page char_count、Tool2 total_template_items 和 Figure CSV count parser 和 validate_sample_run summary/workflow/warning counts 也已采用同样的 fail-closed 规则；面板构建器还会忽略自身 `web/index.html` 的生成差异，避免正常重建误报 dirty。
- 前端视觉回归已实际通过：使用仓库锁定的 `@playwright/test`，在 1440x900 和 390x844 两个 viewport 均通过，期间发现并修复移动端盲区列表/长表导致的横向溢出。宿主系统库未全局安装，本次通过用户态临时依赖包运行，不把它描述成系统环境已永久修复。

### 仍需处理

### 2026-07-17 education output 后续执行增量

- **教育 LLM 输出契约门禁**：Workflow 4 现在校验外部返回的 `metadata`、模式对应的 summary、`suggestions/general_suggestions` 结构；错误 JSON 不会覆盖确定性默认结果，而是回退并标记 `deterministic_fallback`。
- 新增 4 个教育输出契约测试；教育、实验与 dashboard 专项共 `108 passed`，全量回归为 `1342 passed, 20 warnings`。

### 2026-07-17 education input 后续执行增量

- **教育 workflow 输入结构门禁**：`per_reader`、reader payload、`human_statistics`、病例级 `human_metrics` 现在要求对象；错误结构会带字段名 fail-closed，不再在 `.items()/dict()` 处崩溃或被当作空统计。
- 新增 10 个教育输入边界测试；教育、实验和 dashboard 专项共 `104 passed`，全量回归为 `1338 passed, 20 warnings`。

### 2026-07-17 experiment count map 后续执行增量

- **实验状态计数对象门禁**：image-to-text 与 modality recognition 聚合现在要求 source/evidence/modality count map 是“字符串 → 非负整数”对象；错误结构、错误 key 或负数不会再被 `dict()/Counter` 隐式转换。
- 新增 15 个实验状态统计回归测试；实验协议与 dashboard 专项共 `78 passed`，全量回归为 `1328 passed, 20 warnings`。

### 2026-07-17 experiment workflow 后续执行增量

- **实验聚合 workflow2 结构门禁**：`workflows.experiments._case_payloads` 现在要求 `workflow2.cases` 是对象列表；标量、字符串列表或混合错误条目会明确 fail-closed，空列表仍保持兼容。
- 新增 4 个实验聚合回归测试；实验协议与 dashboard 专项共 `63 passed`，全量回归为 `1313 passed, 20 warnings`。

### 2026-07-17 实验证据门禁后续执行增量

- **Experiment gate 证据路径门禁**：`evidence_artifacts/evidence_paths` 现在必须分别是对象列表/字符串列表；错误结构统一返回 `invalid_gate_evidence_contract`，不会在路径遍历中崩溃或错误展示。
- 新增 10 个实验协议回归测试；实验协议与仪表盘专项共 `59 passed`，全量回归为 `1309 passed, 20 warnings`。

### 2026-07-17 Tool5 对齐审计后续执行增量

- **Alignment audit 结构门禁**：Tool5 现在要求 `differences` 是字符串列表、`metrics` 是对象；错误结构会带字段名 fail-closed，不再被 `list()/dict()` 半合法化。
- 新增 8 个 Tool5 回归测试；alignment、工具和完整设计专项共 `308 passed`，全量回归为 `1299 passed, 20 warnings`。

### 2026-07-17 provenance 结构后续执行增量

- **统计链 provenance 对象门禁**：Tool9、Tool10、Tool11、Tool12 对显式 `metadata/provenance` 现在要求对象；字符串、列表、数字和布尔会被安全排除，不再在 `.get()` 处崩溃或误纳入正式统计。
- 新增 40 个 provenance 边界回归测试；工具、重评估、合并专项共 `333 passed`，全量回归为 `1291 passed, 20 warnings`。

### 2026-07-17 OCR provenance 后续执行增量

- **OCR sidecar 身份类型门禁**：candidate sidecar 中显式 `case_id`、`model_key/model/model_name`、`modality` 现在必须是字符串；数字、布尔、列表和对象会进入带字段名的 blocked provenance，不再被 `str()` 转成可评分身份。
- 新增 12 个 OCR provenance 回归测试；OCR、生成 benchmark 与评测专项共 `123 passed`，全量回归为 `1251 passed, 20 warnings`。

### 2026-07-17 统计与缓存链后续执行增量

- **Checkpoint 完整性门禁**：`input_sha256/output_sha256` 现在必须是字符串，异常哈希不会被 `str()` 转换后继续复用缓存。
- **统计聚合对象门禁**：Tool9 排名、Tool10 modelwise、Tool11 hazardwise、Tool12 statistics 对 `composite_inputs/metrics` 的显式值现在必须是对象；错误结构会带字段名 fail-closed，不再在 `.items()` 或 `dict()` 处产生无上下文异常。
- 新增 32 个回归测试；checkpoint、工具聚合和 benchmark evaluation 专项共 `168 passed`，全量回归为 `1239 passed, 20 warnings`。

### 2026-07-17 再后续执行增量

- **legacy 模型配置结构门禁**：`supported_modalities`、`supported_body_parts`、`python_paths` 现在必须是字符串列表；标量、对象和混合列表不再按字符或隐式字符串进入路由。
- **legacy 布尔配置门禁**：`report_trained` 现在必须是真实布尔；`"false"`、`0`、`1` 等值不会再被 `bool()` 静默改变模型可用性。
- 新增 15 个回归测试；路由、legacy 生成器和合同迁移专项共 `235 passed`，全量回归为 `1207 passed, 20 warnings`。

### 2026-07-17 后续执行增量

- **生成器注册表结构门禁**：`generation_parameters` 现在必须是对象，legacy 生成输出中的 `warnings` 必须是字符串列表；错误结构不再被 `dict()`/`list()` 隐式转换。
- **v1 合同迁移顶层对象门禁**：`input`、`human_evaluation`、`legacy_extensions` 及迁移 payload 本身现在必须是对象，异常数据会带字段名 fail-closed，不再产生无上下文的 `ValueError/TypeError`。
- 新增 21 个回归测试；路由、legacy 生成器和合同迁移专项共 `220 passed`，全量回归为 `1192 passed, 20 warnings`。

### 2026-07-17 本轮执行增量

- **Run registry 结构门禁已补齐**：`record_registry_entry/record_run` 现在对 `run_id/stage/status`、`command`、`config/inputs/outputs/metrics`、`warnings` 做显式类型校验；列表、字符串或其他非对象 payload 不再被 `dict()`/`list()` 半合法化写入审计记录。
- **旧 registry 兼容改为 fail-closed**：读取历史 `entries` 时仅保留完整且类型正确的条目，损坏条目会被忽略，不会继续污染后续统计与 dashboard 展示。
- 新增 7 个回归测试覆盖 malformed structured fields 与损坏历史条目；专项 `tests/test_api.py tests/test_workflow_cli.py tests/test_catalog_and_registry.py` 为 `109 passed`，全量回归为 `1171 passed, 20 warnings`。

### 2026-07-17 API 结果结构门禁增量

- **API 工作流结果禁止隐式类型转换**：`/workflow/sample-full`、`/workflow/validate-run`、`/workflow/preflight` 现在对结果对象、路径/摘要/样本对象、错误字符串列表和 `passed` 布尔值做显式结构校验；字符串、整数、列表等 malformed payload 会进入失败 registry 并返回结构化 HTTP 500，不再被 `dict()`、`list()` 或 `bool()` 半合法化。
- 新增 API helper 边界测试及三个 endpoint malformed-result 回归测试；API 专项为 `51 passed`，全量回归为 `1360 passed, 20 warnings`。

### 2026-07-17 API 工作流结果门禁收尾增量

- **single-case 结果结构门禁**：`/workflow/single-case` 现在要求结果、`input` 为对象，错误字段为字符串列表，报告/评估/排名/比较字段为列表；不再把字符串当作可迭代结果或在 `.get()` 处产生无上下文异常。
- **experiments/run 结果结构门禁**：`experiment_count`、`experiments`、`errors` 现在显式校验，错误结构会进入两个相关 registry 的 failed 记录并返回 HTTP 500。
- 新增 2 个 malformed-result endpoint 回归测试；API 专项为 `53 passed`，全量回归为 `1362 passed, 20 warnings`。

### 2026-07-17 API 工作流边界门禁补齐

- **batch-readers / department / analyze-run 结果校验**：三个入口现在对结果对象、非负计数、reader/analysis 对象、错误字符串列表和输出路径字段做显式校验；错误结构不会再被 `list()`、`len()` 或隐式默认值吞掉。
- malformed 结果统一写入对应 failed registry 并返回结构化 HTTP 500；新增 3 个 endpoint 回归测试。
- API 专项为 `56 passed`，全量回归为 `1365 passed, 20 warnings`。

### 2026-07-17 API 合并与展示边界门禁增量

- **merge-batches**：合并结果与 validation 结果现在均要求对象；病例数、失败数、reader 数、`passed` 和错误列表均做 fail-closed 校验。
- **figures/build 与 dashboard/build**：图表计数、面板结果摘要及 registry summary 现在拒绝字符串/列表等 malformed payload，避免错误结果被标记为通过。
- 新增 3 个 endpoint 回归测试；API 专项为 `59 passed`，全量回归为 `1368 passed, 20 warnings`。

### 2026-07-17 API 教育入口边界门禁增量

- **education workflow 结果校验**：教育结果、`mode`、suggestions/general_suggestions 列表和 errors 字符串列表现在显式校验；malformed 返回不会再被 `len()`/`list()` 半合法化。
- 新增教育 endpoint malformed-result 回归测试；API 专项为 `60 passed`，全量回归为 `1369 passed, 20 warnings`。

### 2026-07-17 Dashboard 外部产物结构门禁增量

- **dashboard summary fail-closed**：`run_summary`、`analysis`、`catalog`、`figures`、`run_registry`、`experiments` 及其嵌套 summary/list 字段现在显式校验对象/列表结构；错误的字符串、空列表或空对象不会再被 `or {}` / `or []` 静默当成缺失数据。
- 新增 dashboard malformed payload 回归测试；dashboard/figures/experiments 专项为 `112 passed`，全量回归为 `1376 passed, 20 warnings`。

### 2026-07-17 OCR sidecar provenance 门禁增量

- **OCR 缓存身份字段 fail-closed**：sidecar 显式 `case_id`、源文件 hash、method/provider/model/role/prompt_version 及 verifier 身份字段现在必须是字符串；数字、布尔、列表和对象不会再被 `str()` 隐式转成可复用 provenance。
- 新增 28 个 sidecar provenance 类型回归测试；OCR 专项为 `91 passed`，全量回归为 `1404 passed, 20 warnings`。

### 2026-07-17 Dashboard 模板渲染边界门禁增量

- **模板片段外部列表门禁**：dashboard 在渲染 analysis tables、catalog、experiments、protocol、figures 和 registry 时，显式要求列表/对象结构；字符串、字典等 malformed 历史产物不再被当作可迭代内容直接输出到 HTML。
- web panel/experiments 专项为 `112 passed`，全量回归保持 `1404 passed, 20 warnings`。

### 2026-07-17 Dashboard 列表元素门禁增量

- **列表元素结构校验**：catalog/tools/models、experiments、protocol、figures、registry entries 等渲染列表现在要求每个元素都是对象；字符串/数字等 malformed 元素不再进入 `.get()` 渲染路径导致无上下文崩溃。
- 新增 5 个列表元素回归测试；web panel/experiments 专项为 `117 passed`，全量回归为 `1409 passed, 20 warnings`。

### 2026-07-17 外部证据门禁复核记录

- `medharness2 ocr-benchmark --manifest /tmp/medharness2-missing-ocr-manifest.json --output /tmp/medharness2-ocr-benchmark.json` 实测退出码 `2`，结果 `status=blocked`，阻断项为 `manifest:missing_file`；未把空输入伪装成 benchmark 成功。
- `medharness2 live-smoke --config config/dmx_strong.yaml --output /tmp/live-smoke.json` 实测退出码 `2`，结果为 `status=blocked`、`reason=missing_api_key`、`role=general_judge`；当前环境未提供 `DMX_API_KEY`，因此没有伪造真实 provider 结果。
- `web/index.html` 静态构建和 Python 回归正常；Playwright 视觉回归已通过桌面/移动两项，移动端长表和审计标题的横向溢出已修复。

### 2026-07-17 Dashboard 嵌套列表/路由门禁增量

- **analysis table 行对象校验**：reader、quality-gate、route 等 CSV 行现在必须是对象，错误字符串行不会再在 `.get()` 处崩溃。
- **catalog provider role 校验**：模型角色路由值现在必须是对象，malformed provider 配置会 fail-closed，而不是生成半截前端卡片。
- 新增 3 个回归测试；web/dashboard 专项为 `120 passed`，全量回归为 `1412 passed, 20 warnings`。

### 2026-07-17 CLI 结果边界门禁增量

- **workflow validate-run**：CLI 现在要求 `passed` 为真实布尔、errors/warnings 为字符串列表；字符串 `"true"` 不再被当作通过。
- **workflow preflight**：CLI 现在对结果、sample、paths、routing、blockers、warnings 做显式结构校验；malformed 结果写入 failed registry 并以非零退出，不再在嵌套索引处产生 `KeyError` 或隐式通过。
- 新增 2 个 CLI malformed-result 回归测试；CLI 专项为 `66 passed`，全量回归为 `1414 passed, 20 warnings`。

### 2026-07-17 CLI workflow 收尾增量

- **single-case 结果门禁补齐**：CLI 现在先校验 workflow 返回值为对象，并要求 `generated_reports`、`rankings`、`pairwise_comparisons` 为对象列表、`errors` 为字符串列表；malformed 结果会写入 failed registry 并非零退出，不再把字符串当作可计数报告。
- **analyze-run 结果门禁补齐**：CLI 现在显式校验 `analysis_dir`、7 个计数字段、`errors` 和 `artifacts`；异常结果不会先打印“写入成功”或在缺失字段处触发 `KeyError`。
- 新增 2 个回归测试；CLI 专项为 `68 passed`，全量回归为 `1416 passed, 20 warnings`，并通过 `compileall` 与 `git diff --check`。

### 2026-07-17 类型门禁与 provenance 汇总后续增量

- **Dashboard 布尔字段 fail-closed**：验证通过、真实 OCR 能力、`require_real_ocr` 和工具医学模型要求字段现在必须是真实布尔；malformed 值不会再被 `bool(...)` 渲染成可信健康状态。
- **CLI sample-full / merge-batches 门禁补齐**：两个入口现在先校验 validation、summary、paths、passed 和 errors 的结构；字符串 `"true"` 等值不会再被当作成功，异常统一写入 failed registry。
- **正式 benchmark 仲裁布尔门禁**：hazard adjudication 的 `abstain`、`clinical_validation_required` 及汇总计数拒绝隐式布尔转换；异常仲裁数据不会进入正式统计。
- **生成 benchmark / 重评估 / legacy 路由门禁**：`reference_report_used`、源运行 `require_real_ocr`、legacy `report_trained/source_generation_jsonl` 均改为显式类型检查；Tool9 的 `near_cutoff_review` 也拒绝 malformed 值。
- 新增 49 个边界回归测试；相关专项通过，全量回归为 `1465 passed, 20 warnings`，并通过 `compileall` 与 `git diff --check`。

### 2026-07-17 OCR 与仲裁边界继续收口

- **OCR benchmark 截断统计门禁**：`possible_truncation` 必须是真实布尔，异常字段不会再通过 `bool(...)` 混入模型截断计数。
- **OCR 缓存 verifier 配置门禁**：缓存 sidecar 的 `verifier.configured` 若不是布尔，直接视为不可复用并重新生成，不会把字符串状态解释成路由一致。
- **legacy 批量生成参考报告门禁**：`reference_report` 必须是字符串；异常输入不会被隐式转换后参与 reference-assisted provenance 标记。
- **Tool4 仲裁统计门禁**：`abstain` 统计直接使用已由 Pydantic 合约验证的布尔值，不再二次隐式转换。
- 新增 6 个边界回归测试；全量回归为 `1471 passed, 20 warnings`，并通过 `compileall` 与 `git diff --check`。

### 2026-07-17 合约层严格布尔收口

- **评测合同拒绝 Pydantic 隐式布尔转换**：`ModelProvenance.fallback_used`、hazard judgement/adjudication 的 `abstain`、审计的 `requires_adjudication/requires_review` 等外部产物字段现在使用严格布尔类型；`"false"`、`0`、`1` 和列表/对象不会再被转换成合法状态。
- 新增 6 个合同回归测试；全量回归为 `1477 passed, 20 warnings`，并通过 `compileall` 与 `git diff --check`。

### 2026-07-17 临床标注合约严格布尔收口

- **标注 hazard 字段门禁**：`HazardAnnotation.clinically_significant` 现在使用严格布尔类型；导入的 `0/1`、`"false"/"true"`、列表和对象不会再被 Pydantic 转成合法临床标签。
- 新增标注模型回归测试；全量回归为 `1483 passed, 20 warnings`，并通过 `compileall` 与 `git diff --check`。

### 2026-07-17 临床标注数值合约收口

- **标注数值字段 fail-closed**：`HazardAnnotation.hazard_level` 和 `ReaderAnnotation.confidence` 现在使用严格整数/浮点类型；字符串数字不会再被转换后进入临床 hazard 或一致性统计。
- 新增 2 个标注数值边界回归测试；全量回归为 `1485 passed, 20 warnings`，并通过 `compileall` 与 `git diff --check`。

### 2026-07-17 LLM 与实验响应严格数值门禁

- **Tool4/Tool6 外部响应门禁**：私有 LLM JSON 响应模型的 error index、hazard/clinical level、confidence 和 abstain 字段现在拒绝字符串、浮点整数和隐式布尔转换。
- **实验验证证据门禁**：实验门禁 `passed` 与单项 check `passed` 使用严格布尔类型，错误证据不会再被转换成合法门禁状态。
- 新增 3 个响应结构回归测试；全量回归为 `1488 passed, 20 warnings`，并通过 `compileall` 与 `git diff --check`。

### 2026-07-17 Tool5 alignment 审计置信度类型收口

- **Tool5 外部响应门禁补齐**：`_AuditResponse.confidence` 增加显式 validator，只接受真实 `float`；Pydantic 即使会把整数 `1` 转成 `1.0`，也不再允许这种隐式转换进入 alignment audit 结果。
- 新增字符串、整数、布尔三类非法输入回归测试；专项测试 `13 passed`，全量回归更新为 `1491 passed, 20 warnings`，并通过 `compileall` 与 `git diff --check`。

### 2026-07-17 Tool2 关系索引类型收口

- **Tool2 外部响应门禁补齐**：LLM 抽取关系的 `source_index` / `target_index` 现在使用严格整数类型，拒绝布尔、小数和字符串数字，避免非法关系索引被隐式转换后进入 finding graph。
- 新增 3 类非法索引输入回归测试；全量回归更新为 `1494 passed, 20 warnings`，并通过 `compileall` 与 `git diff --check`。

### 2026-07-17 Tool4/Tool6 置信度类型收口

- **Tool4/Tool6 外部响应门禁补齐**：私有 LLM 响应中的 `confidence` 增加显式真实浮点校验；整数 `1`、字符串数字和布尔值不会再被 Pydantic 隐式转换后进入 hazard adjudication 或 structure audit。
- 新增 Tool4/Tool6 非法 confidence 回归测试；全量回归更新为 `1507 passed, 20 warnings`，并通过 `compileall` 与 `git diff --check`。

### 2026-07-17 OCR 缓存、三模态路由与真实 provenance 收口

- **OCR 缓存 fail-closed**：blocked 或页面契约损坏的 OCR sidecar 不再被重复复用；未跳过页面必须带正数 `char_count`，页数、墨量、状态和质量审计字段均拒绝错误类型。
- **路由统一三大模态**：新增集中式 modality normalizer，将 CXR/CT/MRI 别名统一到 `cxr`、`ct`、`mri`；抽取、预处理、生成、Tool7、质量门和单/双报告工作流共用同一归一化入口，body_part 不再作为硬淘汰条件。
- **真实评测 provenance fail-closed**：benchmark evaluator 不再把布尔、数字、列表或对象 stringify 成 provider/model/role/backend 等合法身份字段，嵌套评测对象和 metadata 也必须保持对象结构。
- 新增 OCR、路由和 provenance 回归覆盖；全量回归更新为 `1556 passed, 20 warnings`，并通过 `compileall` 与 `git diff --check`。

### 2026-07-17 benchmark 统计嵌套对象门禁收口

- **benchmark metrics fail-closed**：`_case_evaluation_metrics` 现在严格要求 candidate、comparison、alignment、hazard、audit 等嵌套字段为对象；不再通过 `dict(...)` 把列表、字符串、布尔值半合法化后参与统计。
- 新增 malformed nested payload 回归测试；全量回归更新为 `1625 passed, 20 warnings`，并通过 `compileall` 与 `git diff --check`。

### 2026-07-17 analyze-run reader percentile 门禁收口

- **reader percentile fail-closed**：`analyze-run` 现在严格要求 `workflow3.reader_percentiles` 及每个 reader 的 percentile 条目为 mapping；列表、字符串、布尔值和数字不会再被 `dict(...)` 半合法化后进入 reader summary。
- 新增 malformed percentile mapping 回归测试；全量回归更新为 `1632 passed, 20 warnings`，并通过 `compileall` 与 `git diff --check`。

### 2026-07-17 教育工作流输入门禁收口

- **Workflow 4 输入 fail-closed**：报告教育建议入口现在严格要求 `human_evaluation`、`likert`、`finding_graph`、`rankings` 及 pairwise comparison 的嵌套对象/列表类型，损坏结构不会再进入教学建议或 hazard 目标选择。
- 新增 malformed education input 回归测试；全量回归更新为 `1648 passed, 20 warnings`，并通过 `compileall` 与 `git diff --check`。

### 2026-07-17 实验汇总嵌套产物门禁收口

- **实验统计 fail-closed**：finding extraction、hazard evaluation 和 educational study 汇总现在严格要求 finding graph、pairwise comparison、hazard errors、suggestions 等字段为正确对象/列表类型，损坏产物不会再被当作空统计。
- 新增实验汇总 malformed payload 回归测试；全量回归更新为 `1656 passed, 20 warnings`，并通过 `compileall` 与 `git diff --check`。

### 2026-07-17 reevaluate-run 病例列表门禁收口

- **Workflow 3 输入 fail-closed**：`reevaluate-run` 现在严格要求源 `workflow2.cases` 为对象列表；字符串、标量和混合列表不会再被 `len()`/迭代路径半合法化或污染失败分母。
- **单病例 composite 输入门禁**：`run_single_case` 现在严格要求每个生成评估的 `composite_inputs` 为对象，损坏的字符串/列表不会再进入 finding coverage 和排名输入。
- 复用已有 reevaluate 回归覆盖并新增单病例回归；专项 `29 passed`，全量回归更新为 `1661 passed, 20 warnings`。

### 2026-07-17 真实医生标注交接与研究 manifest 准备

- 从当前 52 例运行产物确定性筛出覆盖 `cxr/ct/mri` 的 10 例，生成盲化双 reader 标注包；模型身份映射隔离，不进入 reader 包。
- 新增 `research prepare-manifests`，生成 OCR 候选双次比较和论文实验 manifest；当前 OCR winner 保持 `blocked`，论文 formal claim 保持 `false`。
- 新增真实医生交接文档，明确双读、adjudication、禁止伪造临床标签和完成后的验证命令；全量回归为 `1667 passed, 20 warnings`。
- 保留 `annotation prepare-pilot` 作为 `build-pilot` 兼容别名；交接文档与研究 manifest 可由当前 CLI 重现；全量回归更新为 `1675 passed, 20 warnings`。
...
4. **M12** `live-smoke` 已提供；当前缺少 `DMX_API_KEY`，必须保持 blocked。强配置的 hazard reviewer 已默认执行 2 次一致性重测；真实一致率仍需外部 provider smoke 和冻结集验证。
