...
- **OCR 地基质量门禁**：页级 OCR 现在输出 `quality_status=passed|review_required|blocked`；空结果和疑似截断结果会被标记为 `blocked`，`prepare_sample_dataset` 不再把其缓存路径写入可用 `report_text`。独立 verifier 的 disagreement、失败或非法响应只标记 `review_required`，主 OCR 文本保持不变。
