...
> **2026-07-16 最终验证增量**：补齐 API 与单病例入口的显式 `case_id` 传递、旧第四位置 `report_text` 兼容回归，并校验 reader 总数/有效统计数合约；本轮全量回归为 473 passed、18 warnings。
...
- 回归测试持续新增；2026-07-16 本轮最终全量验证为 `474 passed, 18 warnings`，并已通过 `compileall` 与 `git diff --check`。M9 失败分母已接入 `analyze_run` JSON/Markdown 汇总，workflow2/workflow3 聚合边界及内部一致性也已纳入合约验证；pilot10 标注状态已有可执行验收门禁。
