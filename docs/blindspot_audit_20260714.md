...
> **2026-07-16 后续增量**：OCR preflight 现在识别 `chat_completions`/DMX 等 OpenAI 兼容 provider，并在凭据存在时报告为可执行，而不再误报 `unsupported_llm_provider_for_ocr`；OCR `require_real` 缓存门禁和核心执行路径都只接受明确支持的真实 provider，未知 provider metadata 或直接调用都不能绕过门禁；Tool4 hazard 的 observation/location 已与对齐层统一优先 canonical code；这只是能力门禁，不代表真实 OCR 质量已验证。全量回归测试更新为 389 passed。
...
- 回归测试持续新增；2026-07-16 本轮全量验证为 `389 passed, 17 warnings`，并已通过 `compileall` 与 `git diff --check`。M9 失败分母已接入 `analyze_run` JSON/Markdown 汇总。
