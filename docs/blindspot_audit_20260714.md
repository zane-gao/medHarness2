...
> **2026-07-17 OCR 缓存收尾**：OCR 缓存复用前新增 sidecar 结构门禁；`warnings` 必须是字符串列表，`verifier`/`quality_audit` 必须是对象，`quality_status` 只能是 `passed/review_required/blocked`。损坏缓存现在强制重新生成，不会把错误审计状态带入可用 OCR 文本。OCR 专项回归 `51 passed`，全量回归更新为 `1164 passed, 20 warnings`。
...
- 前端视觉回归已实际尝试：仓库现在声明并锁定 `@playwright/test`，桌面/移动两项均已进入测试入口，但因宿主缺少 `libnspr4.so` 无法启动 Chromium；因此保持“未验证”，不把浏览器启动失败误记为页面功能失败或测试通过。
...
- `web/index.html` 静态构建和 Python 回归正常；视觉 Playwright 仍未执行，当前宿主没有 Chromium 可执行文件及 `libnspr4`，该项保持环境阻断。
