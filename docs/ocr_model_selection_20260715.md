...
| PaddleOCR-VL-1.6 | 独立本地/服务候选 | 已接入可选 `ocr-paddle` extra；缺 `paddleocr` provider 或独立 `paddle` runtime 时分别 fail-closed；真实 Qwen 逐页审计前只能 `review_required` | 官方当前接口是完整 `PaddleOCRVL` 文档解析管线，结果通过 `save_to_markdown()` 导出；适合与 VLM 主 OCR 做独立比较 | 通用文档 benchmark 不等于放射报告临床 CER |
...
- PaddleOCR-VL 按官方完整 pipeline 使用：安装 `ocr-paddle` extra（包含 `paddleocr[doc-parser]`）并另行安装匹配硬件的 PaddlePaddle/推理后端；代码分别检查 `paddleocr.PaddleOCRVL` 与 `paddle` runtime。只加载 VLM 权重不计作完整运行；verifier route ready 也不等于真实 audit 通过。
