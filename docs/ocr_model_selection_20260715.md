...
| PaddleOCR-VL 系列（当前公开文档/版本） | 独立本地/服务候选 | 已接入可选 `paddleocr[doc-parser]` provider adapter（安装 extra：`ocr-paddle`）；未安装依赖时 fail-closed | 官方当前接口是完整 `PaddleOCRVL` 文档解析管线，结果通过 `save_to_markdown()` 导出；适合与 VLM 主 OCR 做独立比较 | 通用文档 benchmark 不等于放射报告临床 CER |
...
- PaddleOCR-VL 按官方完整 pipeline 使用：需安装 `paddleocr[doc-parser]` 及匹配硬件的 PaddlePaddle/推理后端；只加载 VLM 权重不计作完整 PaddleOCR-VL 运行。
