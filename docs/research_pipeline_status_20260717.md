...
- 2026-07-17 真实逐页 OCR 复测：`CR2605290003`（CXR）、`CT2605300030`（CT）、`MR2605270001`（MRI）均使用 Yunwu `qwen3-vl-plus` 完成；医院技术支持稀疏末页被记录为 `skip_reason=non_report_page`，不再误触发截断门禁，三例最终均为 `quality_status=passed`。该结果只是 exploratory Qwen OCR 证据，不改变 Doubao winner blocked 状态。
- 2026-07-17 pilot10 exploratory OCR：首轮 8/10 通过；`pilot-002`、`pilot-004` 的第二页包含报告备注和 `万里云/Wanlicloud` 技术支持尾部，初次被误判为截断。加入技术页脚清理后，两例针对性重试均通过，本轮最终 10/10 `quality_status=passed`。这仍不是双重复、独立 verifier 或正式 winner 证据。
