...
- `ocr-benchmark` 对带 provenance 的 candidate sidecar 同样 fail-closed：`quality_status=blocked` 或 `review_required` 的文本不会进入 CER、数字和否定词统计；传统无 provenance 纯文本 manifest 保持兼容。
